...
function nextStep(current) {
  ...
  if(current === 1){
    if(!passwordVal){ password.setCustomValidity('Required'); password.reportValidity(); return; }
    if(!/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&^_-]).{8,}$/.test(passwordVal)){
      password.setCustomValidity('Password must be 8+ chars with letters, numbers, symbols');
      password.reportValidity(); return;
    }
    if(!confirmVal){ confirmPassword.setCustomValidity('Required'); confirmPassword.reportValidity(); return; }
    if(passwordVal !== confirmVal){ confirmPassword.setCustomValidity('Passwords do not match'); confirmPassword.reportValidity(); return; }
  }
...
function toggleTrainerSection(){
  ...
  trainerSection.style.display = document.getElementById('trainer').checked ? 'block' : 'none';
}
...
function checkUsernameAvailability(){
  ...
  if(username.length<3){ status.textContent=''; status.className='username-availability'; return; }
  setTimeout(()=>{
